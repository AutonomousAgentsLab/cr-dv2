...
class Agent(common.Module):
  ...
  def train(self, data, state=None):
    ...
    mets, outs = self._task_behavior.train(self.wm, start, data['is_terminal'], reward)
    metrics.update(mets)
    outputs.update(outs)
    if self.config.expl_behavior != 'greedy':
      mets, outs = self._expl_behavior.train(start, outputs, data)[-2:]
...
class ActorCritic(common.Module):
  ...
  def train(self, world_model, start, is_terminal, reward_fn):
    ...
    # Compute one-step TD error. To use for prioritization.
    batch_shape = start['stoch'].shape[:2]
    state_feat = seq['feat'][0, :]
    V = tf.stop_gradient(self.critic(state_feat).mean().reshape(batch_shape))
    state_reward = reward[0,:].reshape(batch_shape)
    state_disc = tf.cast(seq['discount'][0, :].reshape(batch_shape), V.dtype)
    TD = tf.stop_gradient(state_reward[:, 0:-1] + tf.multiply(state_disc[:, 0:-1], V[:, 1:]) - V[:, 0:-1])  # [nbatchmembers, batchsize - 1], For all states except the last one.
    outs = {'TD': TD}
